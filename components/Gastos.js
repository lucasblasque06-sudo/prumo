    ...
    <div>
            ...
            <div key={e.id} onClick={() => onEditar(e)} style={{ background: COLORS.paper, border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: "14px 16px", boxShadow: CARD_SHADOW, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ color: COLORS.textSoft, fontSize: 13 }}>✎</span>
                </div>
              </div>
      ...
      {filtrados.length > 0 && !isMobile && (
        <div style={{ background: COLORS.paper, border: `1px solid ${COLORS.border}`, borderRadius: 14, overflow: "hidden", boxShadow: CARD_SHADOW }}>
          <div style={{ display: "grid", gridTemplateColumns: "90px 1.6fr 1fr 1fr 1fr 100px 30px", padding: "12px 20px", borderBottom: `1px solid ${COLORS.border}`, background: "#FBFBFA" }}>
            {["Data", "Descrição", "Etapa", "Categoria", "Fornecedor", "Valor", ""].map((h, i) => (
              <div key={i} style={{ fontSize: 11, fontWeight: 700, color: COLORS.textSoft, textTransform: "uppercase", textAlign: i === 5 ? "right" : "left" }}>{h}</div>
            ))}
          </div>
          {filtrados.map((e) => (
            <div key={e.id} onClick={() => onEditar(e)} style={{ display: "grid", gridTemplateColumns: "90px 1.6fr 1fr 1fr 1fr 100px 30px", padding: "13px 20px", borderBottom: `1px solid ${COLORS.border}`, alignItems: "center", fontSize: 13.5, cursor: "pointer" }} className="hover-row">
              ...
              <div style={{ textAlign: "right", color: COLORS.textSoft, fontSize: 13 }}>✎</div>
            </div>
          ))}
